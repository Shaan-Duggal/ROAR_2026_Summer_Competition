...
class ThrottleController:
    ...
    DEFAULT_SECTION_MU = {0: 3.2, 1: 3.0, 2: 3.4, 3: 3.3, 4: 3.05,
                          6: 3.3, 8: 3.1, 9: 2.1}
    DEFAULT_MU = 2.75
    ...
    def __init__(self):
        ...
        self.max_speed = 305
        self.section_mu = dict(self.DEFAULT_SECTION_MU)
        self.default_mu = self.DEFAULT_MU
        self.brake_authority = 825.0
        self.brake_target_divisor = 675.0
        self.section_brake_authority = {s: self.brake_authority for s in range(10)}
        self.brake_throttle = {s: -1.0 for s in range(10)}
        self.trail_brake_level = 1.0
        self.brake_ticks_continuous = False
        self.brake_ticks_divisor = 3.0
        self.brake_ticks_cap = 8
        self.speed_change_threshold = 2.5
        self.avg_speed_change_per_tick = 2.4
        self.brake_threshold_multiplier = 1.05
        self.speed_up_threshold = 0.95
        self.throttle_increase_multiple = 1.35
        self.light_brake_strong = 0.6
        self.light_brake_gentle = 0.1
        self.accel_base = 170
        self.accel_mid = 185
        self.accel_high = 200
        self.latency_ticks = 6
        self.latency_ticks_s3 = 22.5
        self.mu_new_default = 2.75
        self.mu_new_s3 = 3.65
        self.use_model2 = {s: (s == 3) for s in range(10)}
        self.mu_new_section = {s: None for s in range(10)}
        self.latency_ticks_section = {s: None for s in range(10)}
        ...
        self.location_and_radius = self.load_location_and_radius_data(
            os.path.join(os.path.dirname(__file__), "waypoints", "location_with_radius"))


    def reset_after_teleport(self):
        self.previous_speed = 1.0
        self.brake_ticks = 0
        self.prev_brake = deque([0] * 20, maxlen=20)
        self.prev_throttle = deque([0] * 20, maxlen=20)
    ...
    def run(
        ...
        if self.use_model2.get(current_section, False):
    ...
    def get_throttle_and_brake(
        ...
        speed_data = []
        speed_data.append(
            self.speed_for_turn(1, r1, close_distance, target_speed1, current_speed, current_section)
        )
        speed_data.append(
            self.speed_for_turn(2, r2, mid_distance, target_speed2, current_speed, current_section)
        )
        speed_data.append(
            self.speed_for_turn(3, r3, far_distance, target_speed3, current_speed, current_section)
        )
        ...
        if current_speed > 100:
            if current_section != 9:
                ...
                speed_data.append(
                    self.speed_for_turn(4, r4, close_distance, target_speed4, current_speed, current_section)
                )
            ...
            speed_data.append(
                self.speed_for_turn(5, r5, close_distance, target_speed5, current_speed, current_section)
            )
        ...
        throttle, brake = self.speed_data_to_throttle_and_brake(update, current_section)
    ...
    def speed_data_to_throttle_and_brake(self, speed_data: SpeedData,
        ...
        hard_brake_throttle = self.brake_throttle.get(current_section, -1.0)


        percent_of_max = speed_data.current_speed / speed_data.recommended_speed_now
        avg_speed_change_per_tick = self.avg_speed_change_per_tick
        ...
        speed_up_threshold = self.speed_up_threshold
        throttle_decrease_multiple = 0.7
        throttle_increase_multiple = self.throttle_increase_multiple
        brake_threshold_multiplier = self.brake_threshold_multiplier
        ...
        if percent_of_max > 1:
            ...
            if percent_of_max > 1 + (
                ...
                if self.brake_ticks > 0:
                    ...
                    _b = self.trail_brake_level
                    if self.brake_ticks_continuous and self.brake_ticks < 1.0:
                        _b = min(_b, max(0.0, self.brake_ticks))
                    return hard_brake_throttle, _b

                if self.brake_ticks <= 0 and speed_change < self.speed_change_threshold:
                    _raw = ((speed_data.current_speed
                             - speed_data.recommended_speed_now)
                            / self.brake_ticks_divisor)
                    if self.brake_ticks_continuous:
                        self.brake_ticks = min(self.brake_ticks_cap, _raw)
                    else:
                        self.brake_ticks = min(self.brake_ticks_cap, round(_raw))
                    self.dprint(
                        "tb: tick "
                        + str(self.tick_counter)
                ...
                if percent_of_max > 1.02 or percent_speed_change > (
                    ...
                    return (1, self.light_brake_strong)
                else:
                    return (1, self.light_brake_gentle)
        else:
            self.brake_ticks = 0
            if speed_change >= self.speed_change_threshold:
                self.dprint(
    ...
    def speed_for_turn(
        ...
        authority = self.section_brake_authority.get(current_section, self.brake_authority)
        d = (1 / self.brake_target_divisor) * (target_speed**2) + distance
        max_speed = math.sqrt(authority * d)
    ...
    def get_target_speed(self, radius: float, current_section: int):
        ...
        mu = self.section_mu.get(current_section, self.default_mu)
    ...
    def speed_for_turn_new(
        ...
        if current_speed > 230:
            ...
            a = self.accel_mid
        ticks_without_speed_decrease = self.latency_ticks
        if current_section in [3]:
            ticks_without_speed_decrease = self.latency_ticks_s3
        _lat_override = self.latency_ticks_section.get(current_section)
        if _lat_override is not None:
            ticks_without_speed_decrease = _lat_override
    ...
    def get_target_speed_new(self, radius: float, current_section: int, current_location):
        ...
        mu = self.mu_new_s3 if current_section == 3 else self.mu_new_default
        _mu_override = self.mu_new_section.get(current_section)
        if _mu_override is not None:
            mu = _mu_override
